...
## Conteúdo
 ...
 - **Testes:**
   ...
   - [`Testando se a view da rota "/" renderiza o template correto`](#test-main-router-get-correct-template)
<!---
[WHITESPACE RULES]
...
<div id="test-main-router-get-correct-template"></div>

## `Testando se a view da rota "/" renderiza o template correto`

> Aqui, nós vamos criar um teste automatizado simples para garantir que, quando um usuário acessa a rota `/`, o Django renderiza o template esperado.

Vamos começar criando uma classe chamada `TestRootView` que vai herdar de `SimpleSimpleTestCase`:

[users/tests/test_views.py](../users/tests/test_views.py)
```python
from django.test import SimpleSimpleTestCase


class TestRootView(SimpleSimpleTestCase):
    ...
```

`SimpleSimpleTestCase` é uma classe de teste que:

 - ❌ Não cria banco de dados
 - ❌ Não permite acesso ao ORM
 - ✅ É mais rápida que SimpleTestCase
 - ✅ Ideal para:
   - testes de URLs
   - testes de views simples
   - testes de templates
   - testes de utilitários e lógica pura

Continuando, agora vamos criar um **método de teste** chamada `test_root_view_renders_correct_template()`:
...
Para este teste, precisamos de duas coisas:

 - um cliente HTTP de teste
 - uma classe de teste que forneça helpers de asserção de template
 - **NOTE:** O Django fornece isso através de `SimpleTestCase`.
...
**O que acontece aqui?**
 ...
 - **O Django:**
   - resolve a URL `/`
   - encontra a view `login_view()`
   - executa `render(request, "pages/index.html")`
   - registra o template utilizado na resposta

### `🅰️🅰️🅰️ Assert — Verificando o resultado`

Agora criamos um único `assert`, focando apenas em verificar o template usado:
...
> **O que esse assert garante?**
 ...
 - **Que a view:**
   - chamou render()
   - utilizou exatamente o template `"pages/index.html"`
 - **👉 Se o template for renomeado, removido ou trocado, o teste falha.**
...
```bash
pytest -s -x --cov=. -vv \
users/tests/test_views.py::TestRootView::test_root_view_renders_correct_template
```
